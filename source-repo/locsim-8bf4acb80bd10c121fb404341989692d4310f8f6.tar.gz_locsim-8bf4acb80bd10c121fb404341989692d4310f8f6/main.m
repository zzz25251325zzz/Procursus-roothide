...
static void start_scenario_sim(NSString *path, int delivery, int repeat){
	...
	[simManager stopLocationSimulation];
	[simManager clearSimulatedLocations];
	[simManager loadScenarioFromURL:[NSURL fileURLWithPath:jbroot(path)]];
	[simManager flush];
	[simManager startLocationSimulation];
	post_required_timezone_update();
}
...
int main(int argc, char *argv[], char *envp[]) {
	...
	if (strcasecmp(argv[0], "start") == 0){
		...
		if (plist.length > 0){
			...
			NSDictionary *options = [NSDictionary dictionaryWithContentsOfFile:jbroot(plist)][@"Options"];
			if (options){
			...
			NSXMLParser *xmlParser = [[NSXMLParser alloc] initWithContentsOfURL:[NSURL fileURLWithPath:jbroot(gpx)]];
